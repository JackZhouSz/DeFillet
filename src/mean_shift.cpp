...
namespace DeFillet {
    ...
    void MeanShift::shift_point(easy3d::vec3 &point, const std::vector<easy3d::vec3> &data, double kernel_bandwidth,
                                easy3d::vec3 &shift_point) {
        shift_point = easy3d::vec3(0, 0, 0);
        double tw = 0;  // total weight
        int num = data.size();
        for (int i = 0; i < num; i++) {
            easy3d::vec3 tmp = data[i];
            double d = (point - tmp).norm(); //distance
            double w = kernel_func(d, kernel_bandwidth); // weight
            shift_point += w * tmp;
            tw += w;
        }
    ...
    MeanShift::run(const std::vector<easy3d::vec3> &points, const std::vector<easy3d::vec3> &data,
                   double kernel_bandwidth, double eps) {
        ...
        do {
            ...
            for (int i = 0; i < num; i++) {
                if (!stop_moving[i]) {
                    easy3d::vec3 tmp;
                    shift_point(shift_points[i], data, kernel_bandwidth, tmp);
        ...
        for (int i = 0; i < num; i++) {
            ...
            for (size_t j = 0; j < data.size(); j++) {
                double dis = (data[j] - shift_points[i]).norm();
                if (minn > dis) {
                    minn = dis;
                    idx = j;
                }
    ...
    MeanShift::run(const easy3d::vec3 &point, const std::vector<easy3d::vec3> &data, double kernel_bandwidth,
                   double eps) {
        std::vector<easy3d::vec3> points = {point};
        std::vector<std::pair<easy3d::vec3, int>> res = run(points, data, kernel_bandwidth, eps);
        return res[0];
    }
