...
namespace UTILS {
    inline void array_points_to_igl_points(const std::vector<std::array<double, 3>> &array_points,
                                             Eigen::MatrixXd &igl_points) {
        int nb_points = array_points.size();
        igl_points.resize(nb_points, 3);
        for (int i = 0; i < nb_points; i++) {
            igl_points(i, 0) = array_points[i][0];
            igl_points(i, 1) = array_points[i][1];
            igl_points(i, 2) = array_points[i][2];
        }
    }

    inline void igl_points_to_array_points(const Eigen::MatrixXd &igl_points,
                                             std::vector<std::array<double, 3>> &array_points) {
        array_points.clear();
        int nb_points = igl_points.rows();
        array_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            array_points[i] = {igl_points(i, 1), igl_points(i, 1), igl_points(i, 1)};
        }
    }

    inline void array_points_to_eigen_points(const std::vector<std::array<double, 3>> &array_points,
                                           std::vector<Eigen::Vector3d>& eigen_points) {
        int nb_points = array_points.size();
        eigen_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            eigen_points[i] = Eigen::Vector3d(array_points[i][0], array_points[i][1], array_points[i][2]);
        }
    }

    inline void array_points_to_knn_points(const std::vector<std::array<double, 3>> &array_points,
                                           std::vector<KNN::Point> &knn_points) {
        knn_points.clear();
        int nb_points = array_points.size();
        knn_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            knn_points[i] = KNN::Point(array_points[i][0], array_points[i][1], array_points[i][2]);
        }
    }

    inline void array_points_to_cgal_points(const std::vector<std::array<double, 3>> &array_points,
                                            std::vector<CGAL_Point> &cgal_points) {
        cgal_points.clear();
        int nb_points = array_points.size();
        cgal_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            cgal_points[i] = CGAL_Point(array_points[i][0], array_points[i][1], array_points[i][2]);
        }
    }

    inline void eigen_points_to_knn_points(const std::vector<Eigen::Vector3d>& eigen_points, std::vector<KNN::Point> &knn_points) {
        knn_points.clear();
        int nb_points = eigen_points.size();
        knn_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            knn_points[i] = KNN::Point(eigen_points[i].x(), eigen_points[i].y(), eigen_points[i].z());
        }
    }

    inline void array_points_to_easy3d_points(const std::vector<std::array<double, 3>> &array_points,
                                              std::vector<easy3d::vec3>& easy3d_points) {
        easy3d_points.clear();
        int nb_points = array_points.size();
        easy3d_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            easy3d_points[i] = easy3d::vec3(array_points[i][0], array_points[i][1], array_points[i][2]);
        }
    }

    inline void knn_points_to_array_points(const std::vector<KNN::Point> &knn_points,
                                           std::vector<std::array<double, 3>> &array_points) {
        array_points.clear();
        int nb_points = knn_points.size();
        for (int i = 0; i < nb_points; i++) {
            array_points[i] = {knn_points[i].p[0], knn_points[i].p[1], knn_points[i].p[2]};
        }
    }

    inline void cgal_points_to_array_points(const std::vector<CGAL_Point> &cgal_points,
                                            std::vector<std::array<double, 3>> &array_points) {
        array_points.clear();
        int nb_points = cgal_points.size();
        array_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
            array_points[i] = {cgal_points[i].x(), cgal_points[i].y(), cgal_points[i].z()};
        }
    }

    inline void cgal_points_to_knn_points(const std::vector<CGAL_Point> &cgal_points, std::vector<KNN::Point> &knn_points) {
        knn_points.clear();
        int nb_points = cgal_points.size();
        knn_points.resize(nb_points);
        for (int i = 0; i < nb_points; i++) {
    ...
    inline void cgal_points_to_easy3d_points(const std::vector<CGAL_Point>& cgal_points,
                                             std::vector<easy3d::vec3>& easy3d_points) {
        easy3d_points.clear();
        int nb_points = cgal_points.size();
        ...
        easy3d_points.resize(nb_points);
        for(int i = 0; i < nb_points; i++) {
            easy3d_points[i] = easy3d::vec3(cgal_points[i].x(), cgal_points[i].y(), cgal_points[i].z());
        }
    }

    inline void eigen_points_to_easy3d_points(const std::vector<Eigen::Vector3d>& eigen_points,
                                             std::vector<easy3d::vec3>& easy3d_points) {
        easy3d_points.clear();
        int nb_points = eigen_points.size();
        ...
        easy3d_points.resize(nb_points);
        for(int i = 0; i < nb_points; i++) {
            easy3d_points[i] = easy3d::vec3(eigen_points[i].x(), eigen_points[i].y(), eigen_points[i].z());
        }
    }

    inline void cgal_points_to_eigen_points(const std::vector<CGAL_Point>& cgal_points,
                                             std::vector<Eigen::Vector3d>& eigen_points) {
        eigen_points.clear();
        int nb_points = cgal_points.size();
        ...
        eigen_points.resize(nb_points);
        for(int i = 0; i < nb_points; i++) {
            eigen_points[i] = Eigen::Vector3d(cgal_points[i].x(), cgal_points[i].y(), cgal_points[i].z());
        }
    }

    inline void extract_vertex_graph_from_mesh(const std::vector<Eigen::Vector3d>& points,
                                           const std::vector<std::vector<size_t>>& faces,
                                           std::vector<std::vector<std::pair<int,double>>>& graph) {
        int nb_v = points.size();
        ...
        for(size_t i = 0; i < faces.size(); i++) {
            ...
            for(size_t j = 0; j < num; j++) {
                int s = faces[i][j], t = faces[i][(j + 1) % num];
                mp[s].insert(t);
                mp[t].insert(s);
            }
        }
        ...
        for(auto& item : mp) {
            ...
            for(auto& y : item.second) {
                double len = (points[x] - points[y]).norm();
                graph[x].emplace_back(std::make_pair(y, len));
            }
        }
}

    inline void extract_edge_from_mesh(const std::vector<Eigen::Vector3d>& points,
                                       const std::vector<std::vector<size_t>>& faces,
    ...
    inline void extract_k_ring_neighboring(std::vector<std::vector<std::pair<int,double>>>& graph,
                                           int u,
                                           int k,
        ...
        for(size_t i = 0; i < graph[u].size(); i++) {
            ...
            extract_k_ring_neighboring(graph, v, k - 1, neighbors);
        }
    }

    inline double threshold_otsu(std::vector<double>& data, int nb_bin = 255) {
    ...
    inline void vector_normalize(std::vector<double>& data) {
